...
void shortestPathByTransfer(Graph *g, int start, int end) {
    ...
    EdgeNode **edges = (EdgeNode**)malloc(edgeNodeCount * sizeof(EdgeNode*));
    int *origWeights = (int*)malloc(edgeNodeCount * sizeof(int));
    int idx = 0;
    for (int i = 0; i < g->vertexNum; i++) {
        EdgeNode *e = g->vertices[i].firstEdge;
        while (e) {
    ...
    if (dist[end] == INT_MAX) {
        ...
        int transfer = dist[end] - 1; // 换乘次数 = 边数 - 1
        printf("最少换乘路径（换乘 %d 次）：", transfer);
        printPath(g, prev, start, end);
        printf("\n");
    }
    free(dist);
